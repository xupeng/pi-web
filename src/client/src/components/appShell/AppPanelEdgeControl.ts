...
export class AppPanelEdgeControl extends LitElement {
  ...
  static override styles = css`
    ...
    .resize-handle { position: absolute; inset: 0 -8px; z-index: 0; cursor: col-resize; touch-action: none; outline: none; }
    ...
    .edge-button { font: inherit; position: relative; z-index: 1; box-sizing: border-box; display: grid; place-items: center; width: 18px; height: 48px; padding: 0; border: 1px solid var(--pi-border-muted); border-radius: 999px; background: var(--pi-bg); color: var(--pi-muted); opacity: .48; transform: scale(.98); transition: color .12s ease, background .12s ease, opacity .12s ease, transform .12s ease; cursor: pointer; }
    :host(:hover) .edge-button, :host(:focus-within) .edge-button, :host([resizing]) .edge-button, .resize-handle:hover ~ .edge-button, .resize-handle:focus ~ .edge-button, .edge-button:hover, .edge-button:focus-visible { color: var(--pi-text); background: var(--pi-surface-hover); opacity: 1; transform: scale(1); }
